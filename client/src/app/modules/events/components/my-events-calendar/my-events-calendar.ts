...
export class MyEventsCalendarComponent implements OnInit {
    ...
    getTagColorClass(tagName: string): { bg: string, text: string, border: string } {
    const normalizedTag = (tagName || '').toLowerCase();
    switch (normalizedTag) {
        case 'tech':
            return { bg: 'bg-sky-100', text: 'text-sky-700', border: 'border-sky-500' };
        case 'business':
            return { bg: 'bg-indigo-100', text: 'text-indigo-700', border: 'border-indigo-500' };
        case 'art':
