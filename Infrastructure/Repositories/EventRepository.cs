...
public class EventRepository : IEventRepository
{
    ...
    public async Task<List<Event>> GetPublicEventsAsync(CancellationToken cancellationToken) => 
            await _context.Events.Where(e => e.IsPublic).ToListAsync(cancellationToken);

    public async Task<Event?> GetEventByIdAsync(int eventId, CancellationToken cancellationToken) =>
           await _context.Events.Include(e => e.Host).FirstOrDefaultAsync(e => e.Id == eventId, cancellationToken);
    ...
    {
        var userEvent = await _context.EventParticipants.FirstOrDefaultAsync(
            ue => ue.UserId == userId && ue.EventId == eventId, cancellationToken);
